 ...
 */
const MAX_NEW_CARDS_PER_DAY = 40;
 ...
 *   B) NUEVAS   — vencidas con state = 'new', limitadas a MAX_NEW_CARDS_PER_DAY.
 *
 * Ambas llevan .eq('user_id', userId) explícito. Es redundante con RLS a
 ...
 *
 * Los dos conjuntos se unen y se barajan (práctica intercalada): así el
 * orden deja de ser predecible y no se memoriza por posición en la cola.
 */
export function useDueFlashcards() {
  return useQuery({
    ...
    queryFn: async (): Promise<FlashcardWithProgress[]> => {
      ...
      // ── Query B: NUEVAS (topeadas) ───────────────────────────────────────
      const newCardsPromise = supabase
        .from('user_flashcard_progress')
        .select(PROGRESS_SELECT)
        .eq('user_id', userId)
        .lte('due_date', nowUtc)
