...
function EvoChart({ data }: { data: Array<{ date: string; accuracy: number; total: number }> }) {
  if (!data || data.length < 3) {
    return (
      <p className="py-6 text-center text-[12px] text-slate-500 dark:text-white/30">
  ...
  return (
    <div>
      <div className="mb-2 flex items-baseline justify-between">
        <p className="text-[11px] text-slate-500 dark:text-white/30">Últimos 30 días</p>
      ...
      <div className="mt-1 flex justify-between">
        <p className="text-[10px] text-slate-500 dark:text-white/25">{first?.date ? first.date.slice(5) : ''}</p>
...
export default function DashboardPage() {
  ...
  const cardBase = 'rounded-xl border border-indigo-300/50 bg-white/70 backdrop-blur-md dark:border-white/[0.08] dark:bg-white/[0.04]';
    ...
    <div className="relative min-h-screen bg-[#e9e3fb] dark:bg-[#0a0a14]">
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-4 pb-24 pt-6 md:pb-10 lg:px-6">
        ...
        <div className="mb-5">
          ...
          <p className="mt-1 text-[13px] text-slate-500 dark:text-white/40">{today}</p>
        </div>
        ...
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:gap-6">
          ...
          <div className="flex-1 space-y-5">
            ...
            <div className={`relative overflow-hidden p-5 md:p-6 ${cardBase} ${
              recommendation.isNewUser
                ? 'border-violet-300/50 dark:border-violet-400/20'
                : 'border-indigo-400/35 bg-indigo-500/12 dark:border-indigo-400/20 dark:bg-indigo-500/10'
            }`}>
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-indigo-400/10 to-purple-400/6" />
              <div className="relative">
                {/* Etiqueta */}
                <p className="mb-3 text-[10px] font-medium uppercase tracking-wider text-indigo-600/90 dark:text-indigo-400/60">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="mb-4 text-[13px] text-slate-600 dark:text-white/50">
                    ...
                    {/* Tiempo estimado */}
                    <p className="mb-5 text-[12px] text-slate-500 dark:text-white/35">
                  ...
                  {/* Ícono del modo */}
                  <div className="hidden flex-shrink-0 md:flex h-14 w-14 items-center justify-center rounded-2xl border border-indigo-300/50 bg-white/55 text-indigo-500 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-indigo-400">
            ...
            {progressMetrics.length > 0 && (
              <div>
                <p className="mb-2 text-[11px] font-medium uppercase tracking-wider text-slate-500 dark:text-white/30">
                  Mi progreso
                </p>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 animate-slide-up">
                  {progressMetrics.map((m) => (
                    <div key={m.label} className={`${cardBase} p-3`}>
                      <p className="mb-1 text-[11px] text-slate-500 dark:text-white/30">{m.icon} {m.label}</p>
                      <p className={`text-xl font-medium ${metricColorMap[m.color] ?? 'text-indigo-500'}`}>{m.value}</p>
                      <p className="mt-0.5 text-[10px] text-slate-500 dark:text-white/25">{m.sub}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-white/35">Aprende lo que más necesitas.</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-white/35">Estudia exactamente lo que quieras.</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-white/35">Evalúa tu preparación.</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-white/35">Evita olvidar lo aprendido.</p>
                  </div>
                </Link>
              </div>
            </div>
          ...
          <div className="hidden w-[252px] flex-shrink-0 space-y-4 md:block">
            ...
            {countdown && (
              <div className={`${cardBase} p-4`}>
                <p className="mb-2 text-[11px] font-medium uppercase tracking-wider text-slate-500 dark:text-white/30">Cuenta regresiva</p>
                {countdown.days === 0 ? (
                  ...
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className="text-[12px] text-slate-500 dark:text-white/40">{countdown.label}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {/* Racha */}
            <div className={`${cardBase} p-4`}>
              <p className="mb-1 text-[11px] font-medium uppercase tracking-wider text-slate-500 dark:text-white/30">🔥 Racha</p>
              <p className="text-3xl font-medium text-indigo-500 dark:text-indigo-400">
                {isLoading ? '—' : stats?.streakDays ?? 0}
              </p>
              <p className="mt-0.5 text-[11px] text-slate-500 dark:text-white/30">días seguidos</p>
            </div>
            ...
            {stats?.specialtyStats?.length ? (() => {
              ...
              return (
                <div className={`${cardBase} p-4`}>
                  <p className="mb-1 text-[11px] font-medium uppercase tracking-wider text-slate-500 dark:text-white/30">🏆 Mejor especialidad</p>
                  <p className="text-[14px] font-medium text-emerald-600 dark:text-emerald-400">{shortName(best.name)}</p>
                  <p className="text-[12px] text-slate-500 dark:text-white/30">{best.accuracy}% de aciertos</p>
                </div>
              );
            })() : null}
            ...
            {stats?.specialtyStats?.length ? (() => {
              ...
              return (
                <div className={`${cardBase} p-4`}>
                  <p className="mb-1 text-[11px] font-medium uppercase tracking-wider text-slate-500 dark:text-white/30">⚠️ A reforzar</p>
                  <p className="text-[14px] font-medium text-amber-600 dark:text-amber-400">{shortName(worst.name)}</p>
                  <p className="text-[12px] text-slate-500 dark:text-white/30">{worst.accuracy}% de aciertos</p>
                </div>
              );
            })() : null}
            ...
            {/* Banco */}
            <div className={`${cardBase} p-4`}>
              <p className="mb-1 text-[11px] font-medium uppercase tracking-wider text-slate-500 dark:text-white/30">📚 Banco</p>
              <p className="text-3xl font-medium text-amber-500 dark:text-amber-400">
                {isLoading ? '—' : stats?.totalQuestions ?? 0}
              </p>
              <p className="mt-0.5 text-[11px] text-slate-500 dark:text-white/30">preguntas importadas</p>
            </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:hidden">
          {countdown && (
            <div className={`${cardBase} p-3`}>
              <p className="mb-1 text-[10px] uppercase tracking-wider text-slate-500 dark:text-white/30">📅 ENARM</p>
              <p className="text-xl font-medium text-indigo-500 dark:text-indigo-400">{countdown.days}</p>
              <p className="text-[10px] text-slate-500 dark:text-white/30">días restantes</p>
            </div>
          )}
          <div className={`${cardBase} p-3`}>
            <p className="mb-1 text-[10px] uppercase tracking-wider text-slate-500 dark:text-white/30">📚 Banco</p>
            <p className="text-xl font-medium text-amber-500 dark:text-amber-400">{stats?.totalQuestions ?? 0}</p>
            <p className="text-[10px] text-slate-500 dark:text-white/30">preguntas</p>
          </div>
        </div>
